...
void setup()
{
  ...
  stepperL1.setMaxSpeed(10000);    // Max Steps per second
  stepperL1.setAcceleration(5000); // Acceleration Limit

  stepperL2.setMaxSpeed(10000);    // Max Steps per second
  stepperL2.setAcceleration(5000); // Acceleration Limit

  stepperL3.setMaxSpeed(10000);    // Max Steps per second
  stepperL3.setAcceleration(5000); // Acceleration Limit
}
...
void loop()
{
  ...
  stepperL1.setCurrentPosition(0); // Set layer 1 current position to 0:
  stepperL2.setCurrentPosition(0); // Set layer 2 current position to 0:
  stepperL3.setCurrentPosition(0); // Set layer 3 current position to 0:
  ...
  while (true)
  {
    ...
    */  
    ...
    L1targetPosition = (((L1cupNumber-1)*30)-((L1currentCup-1)*30)+540)%360-180;   
    L1targetPosition = (L1targetPosition/30)*L1stepsPerCup;    
    stepperL1.move(L1targetPosition);
    

    L2targetPosition = (((L2cupNumber-1)*30)-((L2currentCup-1)*30)+540)%360-180;   
    L2targetPosition = (L2targetPosition/30)*L2stepsPerCup;    
    stepperL2.move(L2targetPosition);
   

    L3targetPosition = (((L3cupNumber-1)*30)-((L3currentCup-1)*30)+540)%360-180;   
    L3targetPosition = (L3targetPosition/30)*L3stepsPerCup;    
    stepperL3.move(L3targetPosition);
   

    while ( stepperL1.distanceToGo() |stepperL2.distanceToGo() | stepperL3.distanceToGo()){    
      stepperL1.run();
      stepperL2.run();
      stepperL3.run();
    };
  
    L1currentCup = L1cupNumber;
    L2currentCup = L2cupNumber;
    L3currentCup = L3cupNumber;
  }
